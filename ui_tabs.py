...
def render_clinical_documentation(data):
    ...
    if isinstance(data, (str, dict)) and ('error' in str(data) or 'error' in data):
        st.error("⚠️ Unable to process clinical documentation")
        st.error("Please check your Anthropic API credits and try again.")
        return
...
def render_case_summary(data):
    ...
    if isinstance(data, (str, dict)) and ('error' in str(data) or 'error' in data):
        st.error("⚠️ Unable to process case summary")
        st.error("Please check your Anthropic API credits and try again.")
        return
